...
PY_PLATFORMS = {'windows': 'win_amd64', 'linux': 'manylinux1_x86_64'}

# Python configs
PY_VERSION = '{}{}'.format(sys.version_info.major, sys.version_info.minor)
PY_PLATFORM = PY_PLATFORMS['windows' if platform.system() == 'Windows' else 'linux']
...
cmd = 'pip --cache-dir="{cache_dir}"  download -r {requirements}'\
    .format(cache_dir=CACHE_DIR, requirements=os.path.join(ROOT_DIR, '..', 'requirements.txt'))
subprocess.check_call(cmd, shell=True)

ZIP_APP_DIR = tempfile.mkdtemp()
REAL_DEPS_DIR = os.path.join(ZIP_APP_DIR, 'deps')
